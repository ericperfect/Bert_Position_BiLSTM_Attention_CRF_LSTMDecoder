...
class BaseProcessor(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.label_word2idx: Dict[str, int] = kwargs.get('label2idx', {})
        self.idx2label_word: Dict[int, str] = dict([(v, k) for (k, v) in self.label_word2idx.items()])
    ...
    def analyze_corpus_word(self,
                       corpus: Union[List[List[str]]],
                       labels: Union[List[List[str]], List[str]],
                       labels_word: Union[List[List[str]], List[str]],
                       min_count: int = 3,
                       force: bool = False):
        rec_len = sorted([len(seq) for seq in corpus])[int(0.95 * len(corpus))]
        self.dataset_info['RECOMMEND_LEN'] = rec_len
        ...
        if len(self.label2idx) == 0 or force:
            self._build_label_dict(labels)
        if len(self.label_word2idx) == 0 or force:
            self._build_label_word_dict(labels_word)
    ...
    def _build_label_word_dict(self, corpus: Union[List[List[str]], List[str]]):
        raise NotImplementedError
    ...
    def numerize_label_word_sequences(self,
                                 sequences: List[List[str]]) -> List[List[int]]:
        raise NotImplementedError

    def reverse_numerize_label_word_sequences(self, sequence, **kwargs):
        raise NotImplementedError
