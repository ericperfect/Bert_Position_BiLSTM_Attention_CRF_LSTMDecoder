...
class LabelingProcessor(BaseProcessor):
    ...
    def _build_label_word_dict(self,
                          label_list: List[List[str]]):
        """
        Build label2idx dict for sequence labeling task
        ...
        for token in token2count.keys():
            if token not in label_word2idx:
                label_word2idx[token] = len(label_word2idx)

        self.label_word2idx = label_word2idx
        self.idx2label_word = dict([(value, key)
                               for key, value in self.label_word2idx.items()])
        logging.debug(f"build label2idx dict finished, contains {len(self.label2idx)} labels.")
    ...
    def process_z_dataset(self,
        ...
        if subset is not None:
            ...
            target = data[:]
        numerized_samples = self.numerize_label_word_sequences(target)
        padded_seq = pad_sequences(
            numerized_samples, max_len, padding='post', truncating='post')
        return to_categorical(padded_seq, len(self.label_word2idx))
    ...
    def reverse_numerize_label_word_sequences(self,
                                         sequences,
                                         lengths=None):
        result = []
